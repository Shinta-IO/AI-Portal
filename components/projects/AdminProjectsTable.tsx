...
import { Check, ChevronUpDown, Plus, X } from "lucide-react";
...
.from("projects")
.insert({ title: newTitle, description: newDescription, status: "active" })
.select()
.single();
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-zinc-900 p-6 rounded shadow-lg w-full max-w-lg space-y-4">
        ...
        <div>
          ...
          <Listbox value={null} onChange={(profile) => {
            ...
            <div className="relative">
              <Listbox.Button className="w-full bg-zinc-50 dark:bg-zinc-800 border px-3 py-2 rounded text-left">
                ...
                <ChevronUpDown className="w-4 h-4 absolute right-3 top-2.5" />
              </Listbox.Button>
              <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
                <Listbox.Options className="absolute z-10 mt-1 w-full bg-white dark:bg-zinc-900 border rounded shadow-lg max-h-60 overflow-auto">
                  {profiles.map((profile) => (
                    <Listbox.Option key={profile.id} value={profile} className={({ active }) =>
                      clsx("cursor-pointer px-4 py-2", active && "bg-brand/10 dark:bg-brand/20")}>
                      {({ selected }) => (
                        <div className="flex justify-between items-center">
                          <span>{profile.first_name} {profile.last_name}</span>
                          {selectedUsers.find((u) => u.id === profile.id) && <Check className="w-4 h-4 text-green-500" />}
                        </div>
                      )}
                    </Listbox.Option>
                  ))}
          ...
          {selectedUsers.length > 0 && (
            <div className="flex flex-wrap mt-2 gap-2">
              {selectedUsers.map((u) => (
                <div key={u.id} className="flex items-center bg-zinc-200 dark:bg-zinc-800 text-sm px-3 py-1 rounded-full">
                  {u.first_name} {u.last_name}
                  <button className="ml-2 text-zinc-600 hover:text-red-500" onClick={() => removeUser(u.id)}>
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ))}
